...
// ostream &operator<<(ostream &out, card_template_t card_temp) {
//   for (int i = 0; card_temp.size(); ++i) {
//     cout << card_temp[i] << endl;
//   }
//   return out;
// }

void displayGraphics(card_template_t card_temp) { 
  for (size_t i = 0; i < card_temp.size(); i++) {
    cout << card_temp[i] << endl;
  }
}

void displayRowGraphics(vector<card_template_t> card_temp_list, bool border) {
  if (card_temp_list.size() > 0) {
    for (size_t i = 0; i < card_temp_list[0].size(); i++) { // max = 12
      for (size_t j = 0; j < card_temp_list.size(); j++) { // max = 2
        if (border && j == 0) { cout << EXTERNAL_BORDER_CHAR_UP_DOWN; }
        cout << card_temp_list[j][i];
        if (border && j == card_temp_list.size() - 1) { cout << EXTERNAL_BORDER_CHAR_UP_DOWN; }
      }
      cout << endl;
    }
  }
...
card_template_t minionTemplate(std::shared_ptr<Card> minion) {
  // card_template_t display_minion_activated_ability(std::string name,int cost,int attack,int defence,
  //                                                int ability_cost, std::string ability_desc);
  card_template_t minion_template;
  if (minion->getAbility().getType() == AbilityType::NONE) {
    minion_template = display_minion_no_ability(minion->getName(), 
                                       minion->getMagicCost(), 
                                       minion->getAttack(), 
                                       minion->getDefense());

  } else if (minion->getAbility().getType() == AbilityType::PASSIVE) {
    minion_template = display_minion_triggered_ability(minion->getName(), 
                                              minion->getMagicCost(), 
                                              minion->getAttack(),
                                              minion->getDefense(),
                                              minion->getAbility().getDescription());
  } else if (minion->getAbility().getType() == AbilityType::ACTIVE) {
    minion_template = display_minion_activated_ability(minion->getName(),
                                              minion->getMagicCost(),
                                              minion->getAttack(),
                                              minion->getDefense(),
  ...
  return minion_template;

}

card_template_t spellTemplate(std::shared_ptr<Card> spell) {
  // card_template_t display_minion_activated_ability(std::string name,int cost,int attack,int defence,
  //                                                int ability_cost, std::string ability_desc);
  card_template_t spell_template;
  spell_template = display_spell(spell->getName(), 
                                       spell->getMagicCost(), 
...
card_template_t enchantTemplate(std::shared_ptr<Card> enchant) {
  card_template_t enchant_template;
  if (enchant->getAttackDesc() == "") {
    enchant_template = display_enchantment(enchant->getName(),
                                         enchant->getMagicCost(),
                                         enchant->getAbility().getDescription());
  } else {
    enchant_template = display_enchantment_attack_defence(enchant->getName(), 
                                                          enchant->getMagicCost(),
                                                          enchant->getAbility().getDescription(), 
                                                          enchant->getAttackDesc(), 
                                                          enchant->getDefenseDesc());
  }
  return enchant_template;
}

card_template_t ritualTemplate(std::shared_ptr<Card> ritual) {
  card_template_t ritual_template;
  ritual_template = display_ritual(ritual->getName(), 
                                   ritual->getMagicCost(), 
